 ...
 */
export function detectProjectType(userPrompt) {
 ...
 */
export function extractProjectName(userPrompt) {
 ...
 */
export function extractRequirements(userPrompt, _projectType) {
    const requirements = [];
 ...
 */
export function extractTechStack(prompt, researchData) {
    const stack = new Set();
 ...
 */
export async function loadTemplate(projectType, templatePath) {
    ...
    if (templatePath) {
        try {
            ...
            throw new PRDGenerationError(`Failed to load custom template from ${templatePath}`, error instanceof Error ? error : undefined);
        }
    }
    // Template mapping
    const templateMap = {
 ...
 */
function getDefaultTemplate(_projectType) {
 ...
 */
function formatRequirementsByPriority(requirements, priority) {
 ...
 */
function formatBrandingGuidelines(branding) {
    if (!branding) {
 ...
 */
function formatTechnicalDocs(docs) {
    if (!docs) {
 ...
 */
export function fillTemplate(template, options, requirements, techStack) {
    let filled = template;
    ...
    const replacements = {
        ...
        '{{TECH_STACK}}': techStack.length > 0 ? techStack.map((t) => `- ${t}`).join('\n') : '- To be determined',
 ...
 */
export async function generatePRD(options) {
    try {
        ...
        const content = fillTemplate(template, { ...options, projectType, projectName }, requirements, techStack);
        // Step 7: Determine output path
        const timestamp = Date.now();
        const slug = projectName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
        const outputPath = options.outputPath || path.join('.taskmaster', 'docs', `prd-${slug}-${timestamp}.md`);
        ...
        throw new PRDGenerationError('Failed to generate PRD', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export function generatePRDContent(options) {
    const projectType = options.projectType || detectProjectType(options.userPrompt);
    const projectName = options.projectName || extractProjectName(options.userPrompt);
    const requirements = extractRequirements(options.userPrompt, projectType);
    const techStack = extractTechStack(options.userPrompt, options.researchData);

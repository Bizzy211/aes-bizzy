 ...
 */
export function parseDocumentation(content) {
    const headings = [];
    const codeBlocks = [];
    const lists = [];
 ...
 */
export function estimateTokenUsage(query, options) {
 ...
 */
function extractColorSchemes(content) {
    const colors = [];
    // Match hex colors
 ...
 */
function extractCodeExamples(content, language) {
    const examples = [];
 ...
 */
function extractPatternsAndPractices(content) {
    const patterns = [];
    const bestPractices = [];
    // Pattern indicators
    ...
    const practiceMatches = content.match(/(?:best practice|recommendation|tip|should|always|never):\s*([^.!?\n]+)/gi);
    if (practiceMatches) {
 ...
 */
export async function searchBranding(query, options = {}) {
 ...
 */
export async function searchTechDocs(query, options = {}) {
 ...
 */
export async function gatherProjectResearch(projectQuery, options = {}) {
    const sources = [];
    const errors = [];
    let totalTokens = 0;
    const result = {
...
async function callExaWebSearch(params) {
    ...
    throw new ResearchError('callExaWebSearch should be replaced with mcp__exa__web_search_exa in skill context', 'exa');
}
async function callExaGetCodeContext(params) {
...
- yarn add another-lib
    ...
    throw new ResearchError('callExaGetCodeContext should be replaced with mcp__exa__get_code_context_exa in skill context', 'exa');
}
async function callRefSearchDocumentation(params) {
    // Placeholder for actual MCP call
    if (process.env.NODE_ENV === 'test' || process.env.MOCK_MCP === 'true') {
        return {
            results: [
    ...
    throw new ResearchError('callRefSearchDocumentation should be replaced with mcp__ref__ref_search_documentation in skill context', 'ref');
}
async function callRefReadUrl(params) {
    // Placeholder for actual MCP call
    if (process.env.NODE_ENV === 'test' || process.env.MOCK_MCP === 'true') {
        return {
    ...
    throw new ResearchError('callRefReadUrl should be replaced with mcp__ref__ref_read_url in skill context', 'ref');
}
// ============================================================================
// Exports
// ============================================================================

...
import { isDockerInstalled, isDockerRunning, getContainerStatus, isQdrantHealthy, getQdrantLogs, } from './docker-manager.js';
 ...
 */
export async function checkQdrant(containerName = 'qdrant-heimdall') {
    const start = Date.now();
 ...
 */
export async function checkQdrantApi(url = 'http://localhost:6333') {
    const start = Date.now();
    try {
 ...
 */
export async function checkCollection(url = 'http://localhost:6333', collectionName = 'claude-memories') {
    const start = Date.now();
    try {
 ...
 */
export async function checkHeimdallConfig() {
    const start = Date.now();
 ...
 */
export async function runFullHealthCheck(config = DEFAULT_HEIMDALL_CONFIG) {
    const checks = [];
    const recommendations = [];
    // Run all checks in parallel where possible
    const [dockerCheck, qdrantCheck, apiCheck, embeddingCheck, collectionCheck, mcpCheck, configCheck,] = await Promise.all([
        checkDocker(),
        checkQdrant(),
    ...
    checks.push(dockerCheck, qdrantCheck, apiCheck, embeddingCheck, collectionCheck, mcpCheck, configCheck);
    ...
    const qdrantStatus = await getContainerStatus();
    const memoryStats = await getMemoryStats(config);
    const heimdallStatus = {
        status: overall,
        qdrant: qdrantStatus,
        embedding: {
 ...
 */
export async function isHeimdallOperational() {
 ...
 */
export async function getDiagnostics(containerName = 'qdrant-heimdall') {

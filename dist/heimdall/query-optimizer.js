 ...
 */
export function estimateMemoryTokens(memory, includeMetadata = true) {
 ...
 */
export function compressContent(content, maxLength = 500) {
    ...
    const keyLines = lines.filter(line => keywords.some(kw => line.toLowerCase().includes(kw))).slice(0, 3);
    // Combine
 ...
 */
export function deduplicateMemories(memories, threshold = 0.7) {
    ...
    for (const memory of memories) {
        ...
        const isDuplicate = seen.some(existing => textSimilarity(existing, memory.content) > threshold);
        if (!isDuplicate) {
            result.push(memory);
 ...
 */
function calculatePriorityScore(memory, priorityTypes) {
 ...
 */
export function prioritizeMemories(memories, priorityTypes) {
    return [...memories].sort((a, b) => calculatePriorityScore(b, priorityTypes) - calculatePriorityScore(a, priorityTypes));
}
// ============================================================================
// Optimized Query
// ============================================================================
 ...
 */
export async function optimizedQuery(query, options = {}) {
    const { maxTokens = 2000, priorityTypes, minRelevance = 0.6, maxMemories = 10, deduplicate = true, deduplicationThreshold = 0.7, compressContent: shouldCompress = true, maxContentLength = 500, includeMetadata = false, outputFormat = 'compact', } = options;
    ...
    const originalTokens = response.memories.reduce((sum, m) => sum + estimateMemoryTokens(m, includeMetadata), 0);
 ...
 */
export function formatMemories(memories, format = 'compact', includeMetadata = false) {
 ...
 */
function formatFull(memories, includeMetadata) {
    ...
    for (const memory of memories) {
        ...
        if (includeMetadata) {
            ...
            if (analysis.agent)
                lines.push(`Agent: ${analysis.agent}`);
            if (analysis.taskId)
                lines.push(`Task: ${analysis.taskId}`);
 ...
 */
function formatCompact(memories, includeMetadata) {
    const lines = [];
 ...
 */
function formatMinimal(memories) {
 ...
 */
export async function getAgentContext(agentName, taskDescription, maxTokens = 1500) {
    return optimizedQuery(`${agentName} agent context: ${taskDescription}`, {
        maxTokens,
 ...
 */
export async function getErrorContext(errorDescription, maxTokens = 1000) {
    return optimizedQuery(`error resolution: ${errorDescription}`, {
        maxTokens,
 ...
 */
export async function getPatternContext(feature, technology, maxTokens = 1200) {
 ...
 */
export async function buildContextPrompt(agentName, taskTitle, taskDescription, maxTokens = 2000) {
    const result = await getAgentContext(agentName, `${taskTitle}: ${taskDescription}`, maxTokens);

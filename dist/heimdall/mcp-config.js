 ...
 */
export function findAllConfigLocations() {
    const locations = [];
 ...
 */
function checkConfigHasHeimdall(configPath) {
    try {
        ...
        return !!(config.mcpServers?.heimdall ||
 ...
 */
export function getPrimaryConfigLocation() {
 ...
 */
export function generateHeimdallMcpEntry(heimdallConfig = DEFAULT_HEIMDALL_CONFIG) {
    return {
        command: 'npx',
 ...
 */
export function addHeimdallToConfig(configPath, heimdallConfig = DEFAULT_HEIMDALL_CONFIG, serverName = 'heimdall') {
    try {
 ...
 */
export function removeHeimdallFromConfig(configPath) {
    try {
 ...
 */
export function updateHeimdallConfig(configPath, updates) {
    try {
        const config = readMcpConfig(configPath);
        if (!config?.mcpServers) {
        ...
        // Find Heimdall entry
        const heimdallKey = Object.keys(config.mcpServers).find(key => key === 'heimdall' || key === 'heimdall-mcp' || key.includes('heimdall'));
        if (!heimdallKey) {
 ...
 */
export function setHeimdallEnabled(configPath, enabled) {
    try {
        const config = readMcpConfig(configPath);
        if (!config?.mcpServers) {
            return { success: false, error: 'No MCP configuration found' };
        }
        const heimdallKey = Object.keys(config.mcpServers).find(key => key.includes('heimdall'));
        if (!heimdallKey) {
            return { success: false, error: 'Heimdall not found in configuration' };
        }
 ...
 */
export function syncHeimdallAcrossConfigs(heimdallConfig = DEFAULT_HEIMDALL_CONFIG) {
    const locations = findAllConfigLocations();
    const synced = [];
    const failed = [];
 ...
 */
export function validateHeimdallMcpConfig(configPath) {
    const errors = [];
    const warnings = [];
    const config = readMcpConfig(configPath);
    if (!config) {

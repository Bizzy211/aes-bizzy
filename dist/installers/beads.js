 ...
 */
export async function installBeadsWithMethod(method, options = {}) {
 ...
 */
async function installBeadsBinary(silent) {
...
async function installBeadsBinaryWindows(silent) {
    ...
    if (release) {
        ...
        const windowsAsset = release.assets.find(a => a.name.includes('windows') && a.name.includes('amd64') && a.name.endsWith('.zip'));
        if (windowsAsset) {
 ...
 */
export async function installBeads(options = {}) {
 ...
 */
export async function isBeadsInstalled() {
 ...
 */
export async function installBeadsMCP(silent = false) {
    const hasNpm = await checkCommandExists('npm');
    if (!hasNpm) {
        return {
            success: false,
 ...
 */
export async function getBeadsConfig() {

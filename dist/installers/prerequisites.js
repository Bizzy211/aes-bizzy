...
import { DEFAULT_VERSION_REQUIREMENTS, INSTALL_SUGGESTIONS, } from '../types/prerequisites.js';
/**
 ...
 */
export async function checkPrerequisites(requirements = DEFAULT_VERSION_REQUIREMENTS) {
    ...
    if (!node.installed) {
        ...
        allMet = false;
        suggestions.push(`Node.js version ${node.version} is below minimum required ${requirements.node}. ${getSuggestion('node')}`);
    }
    // Check npm
    if (!npm.installed) {
        allMet = false;
        ...
        allMet = false;
        suggestions.push(`npm version ${npm.version} is below minimum required ${requirements.npm}. ${getSuggestion('npm')}`);
    }
    // Check Git
    if (!git.installed) {
        allMet = false;
        ...
        allMet = false;
        suggestions.push(`Git version ${git.version} is below minimum required ${requirements.git}. ${getSuggestion('git')}`);
    }
    ...
    if (!claudeCode.installed) {
        ...
        suggestions.push(`(Optional) Claude Code version ${claudeCode.version} is below recommended ${requirements.claudeCode}. ${getSuggestion('claudeCode')}`);
    }
    return {
        node,
        npm,
 ...
 */
export async function hasMinimumPrerequisites() {
 ...
 */
export function formatPrerequisitesReport(result) {
    const lines = ['Prerequisites Check:', ''];
    const formatStatus = (name, status, required) => {

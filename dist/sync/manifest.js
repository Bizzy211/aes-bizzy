 ...
 */
export async function loadManifestByTier(repoPath, tier) {
 ...
 */
export async function discoverManifests(repoPath) {
    const result = {
        essential: null,
        recommended: null,
        full: null,
 ...
 */
export function filterByCategories(manifest, categories) {
 ...
 */
export function filterRequired(manifest) {
    return manifest.files.filter((file) => file.required === true);
}
/**
 * Convert manifest files to ComponentFile format for sync
 */
export function manifestFilesToComponentFiles(manifestFiles, repoPath) {
 ...
 */
export function inferComponentType(filePath) {
 ...
 */
export function resolveDependencies(selectedFiles, manifest) {
    const allFiles = new Map(manifest.files.map((f) => [f.path, f]));
    const resolved = new Set();
    const result = [];
    function addWithDependencies(file) {

 ...
 */
async function getAllFiles(dirPath, basePath = dirPath, includeHidden = true) {
    const files = [];
 ...
 */
async function copyFile(sourcePath, destDir, relativePath) {
 ...
 */
export async function createBackup(reason, options = {}) {
 ...
 * Clean old backups
 */
export async function cleanOldBackups(options = {}) {
    const opts = { ...DEFAULT_BACKUP_OPTIONS, ...options };
    const backups = await listBackups();
    if (backups.length === 0) {
        return {
    ...
    logger.success(`Cleaned up ${removedBackups.length} old backups, freed ${formatSize(freedSpace)}`);
    return {
        success: true,
 ...
 */
function deepMergeObjects(existing, incoming, preferExisting, conflicts, path = '') {
    ...
    for (const key of Object.keys(incoming)) {
        ...
        if (!(key in existing)) {
            ...
            result[key] = deepMergeObjects(existingVal, incomingVal, preferExisting, conflicts, fullKey);
        }
 ...
 */
export function mergeConfigs(existing, incoming, options = {}) {
 ...
 */
export async function mergeSettingsFile(newSettings, options = {}) {
    const opts = { ...DEFAULT_MERGE_OPTIONS, ...options };
    const claudeDir = getClaudeConfigDir();

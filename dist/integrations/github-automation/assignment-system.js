 ...
 */
export function getAutomationLog(limit = 100) {
...
export async function fetchIssue(owner, repo, issueNumber, token) {
    try {
        const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues/${issueNumber}`, { headers: buildHeaders(token) });
        if (!response.ok) {
 ...
 */
export async function fetchOpenIssues(owner, repo, token, options = {}) {
    try {
        ...
        const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues?${params}`, { headers: buildHeaders(token) });
        if (!response.ok) {
 ...
 */
export async function postComment(owner, repo, issueNumber, body, token) {
    try {
 ...
 */
export async function addLabels(owner, repo, issueNumber, labels, token) {
    try {
 ...
 */
export function generateAssignmentComment(agentMatches, analysisKeywords) {
    const lines = [
 ...
 */
export function generateTriageComment(triageResult) {
    const lines = [
        '## 🔍 Issue Triage Report',
        '',
    ];
 ...
 */
export async function assignIssue(owner, repo, issue, token, config = {}) {
    const { confidenceThreshold = 40, autoAssign = true, requireConfirmation = false, } = config;
    const result = {
        success: false,
        issue,
        assignedAgents: [],
    ...
    try {
        ...
        if (autoAssign && !requireConfirmation) {
            const comment = generateAssignmentComment(analysis.agentMatches.slice(0, 3), analysis.extractedKeywords);
 ...
 */
export async function triageIssue(issue, agentsDir) {
    const analysis = await analyzeIssue(issue, agentsDir);
    const firstMatch = analysis.agentMatches[0];
    const requiresManualReview = analysis.agentMatches.length === 0 ||
        !firstMatch ||
        firstMatch.score < 40;
    return {
        issue,
 ...
 * Batch assign issues
 */
export async function batchAssignIssues(owner, repo, issues, token, config = {}) {
    const results = [];
    let assigned = 0;
    let skipped = 0;
    let failed = 0;
 ...
 */
export async function getAssignmentRecommendation(issue, agentsDir) {
    const analysis = await analyzeIssue(issue, agentsDir);
    const firstMatch = analysis.agentMatches[0];
    if (!firstMatch) {
        return {
 ...
 */
export function shouldExcludeIssue(issue, excludeLabels = []) {
 ...
 */
export async function processIssueEvent(action, issue, owner, repo, token, config) {
...
//# sourceMappingURL=assignment-system.js.map
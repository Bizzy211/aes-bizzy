 ...
 */
import { executeCommand, execCommandWithSpinner, checkCommandExists, } from '../utils/shell.js';
 ...
 * });
 */
export async function createBead(options) {
    try {
        ...
        if (options.json !== false && result.stdout) {
            try {
                const bead = JSON.parse(result.stdout);
                return {
                    success: true,
                    id: bead.id,
                    bead,
 ...
 * });
 */
export async function getReadyBeads(options = {}) {
    try {
        const bd = await getBdPath();
        const args = ['ready'];
        // Request JSON output
 ...
 */
export async function updateBeadStatus(beadId, status, options = {}) {
    try {
        const bd = await getBdPath();
 ...
 */
export async function closeBead(beadId, reason, options = {}) {
    try {
        const bd = await getBdPath();
        const args = ['close', beadId, '--reason', reason];
        if (options.json !== false) {
            args.push('--json');
        }
        const result = await executeCommand(bd, args, { silent: true });
 ...
 * Sync beads to git
 */
export async function syncBeads(options = {}) {
    try {
        const bd = await getBdPath();
        const args = ['sync'];
        if (options.silent) {
 ...
 */
export async function showBead(beadId, options = {}) {
    try {
        const bd = await getBdPath();
        const args = ['show', beadId];
        if (options.json !== false) {
            args.push('--json');
        }
        const result = await executeCommand(bd, args, { silent: true });
 ...
 */
export async function assignBead(beadId, agentName, options = {}) {
    try {
        const bd = await getBdPath();
        const agentTag = createAgentTag(agentName);
        const args = ['update', beadId, '--add-tag', agentTag];
        if (options.json !== false) {
            args.push('--json');
        }
        const result = await executeCommand(bd, args, { silent: true });
 ...
 */
export async function unassignBead(beadId, agentName, options = {}) {
    try {
        const bd = await getBdPath();
        const agentTag = createAgentTag(agentName);
        const args = ['update', beadId, '--remove-tag', agentTag];
        if (options.json !== false) {
            args.push('--json');
        }
        const result = await executeCommand(bd, args, { silent: true });

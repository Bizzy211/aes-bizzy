...
import { generateContextTags, createTag, CONTEXT_TAG_PREFIXES, } from '../types/context.js';
// ============================================================================
// Constants
// ============================================================================
 ...
 */
export function getBeadsPath(scope, projectPath) {
    if (scope === 'global') {
 ...
 */
export function getInteractionsPath(scope, projectPath) {
    return path.join(getBeadsPath(scope, projectPath), INTERACTIONS_FILE);
}
/**
 * Get the trash directory path
 */
export function getTrashPath(scope, projectPath) {
 ...
 */
export async function ensureBeadsDir(scope, projectPath) {
 ...
 */
export function isBeadsInitialized(scope, projectPath) {
    const beadsPath = getBeadsPath(scope, projectPath);
...
export async function createContext(options) {
    try {
        ...
        await fs.promises.appendFile(interactionsPath, JSON.stringify(bead) + '\n');
        return {
            success: true,
            bead,
            id: bead.id,
 ...
 */
export async function getContext(beadId, scope = 'project', projectPath) {
 ...
 */
export async function updateContext(beadId, updates, scope = 'project', projectPath) {
    try {
        const beads = await loadAllBeads(scope, projectPath);
 ...
 */
export async function removeContext(beadId, scope = 'project', projectPath) {
    try {
        const beads = await loadAllBeads(scope, projectPath);
        const index = beads.findIndex(b => b.id === beadId);
        if (index === -1) {
 ...
 */
export async function loadAllBeads(scope = 'project', projectPath) {
    const interactionsPath = getInteractionsPath(scope, projectPath);
    if (!fs.existsSync(interactionsPath)) {
        return [];
    }
 ...
 */
export async function writeAllBeads(beads, scope = 'project', projectPath) {
    const interactionsPath = getInteractionsPath(scope, projectPath);
...
export async function searchContext(options) {
    try {
        ...
        if (options.tags && options.tags.length > 0) {
            beads = beads.filter(b => options.tags.every(tag => b.tags.includes(tag)));
        }
        // Full-text search
        if (options.query) {
            const queryLower = options.query.toLowerCase();
            beads = beads.filter(b => b.title.toLowerCase().includes(queryLower) ||
                b.description?.toLowerCase().includes(queryLower) ||
                b.content?.toLowerCase().includes(queryLower));
        }
        // Sort by createdAt descending (newest first)
        beads.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        // Pagination
 ...
 */
export async function listContext(options) {
    try {
        let beads = await loadAllBeads(options.scope || 'project');
        // Apply filters
        if (options.type) {
 ...
 */
export async function listTags(prefix, scope = 'project', projectPath) {
    try {
        const beads = await loadAllBeads(scope, projectPath);
        ...
        for (const bead of beads) {
            for (const tag of bead.tags) {
                ...
                if (prefix) {
                    const prefixValue = CONTEXT_TAG_PREFIXES[prefix];
 ...
 */
export async function findRelated(beadId, scope = 'project', projectPath, limit = 5) {
    const beads = await loadAllBeads(scope, projectPath);
 ...
 */
export async function exportBeadsToJson(scope = 'project', projectPath, filters) {
 ...
 */
export async function importBeadsFromJson(data, options) {
    const scope = options.scope || 'project';
    await ensureBeadsDir(scope);
    ...
    if (options.merge) {
        ...
        const newBeads = data.beads.filter(b => b.id && b.title && b.contextType).map(b => ({ ...b, scope }));

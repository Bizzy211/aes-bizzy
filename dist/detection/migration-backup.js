 ...
 */
export async function createPreMigrationBackup(reason, migrationType = 'full', conflicts = []) {
    ...
    // Write manifest
    await fs.writeFile(path.join(backupPath, 'manifest.json'), JSON.stringify(manifest, null, 2), 'utf-8');
 ...
 */
export async function getMigrationBackup(backupId) {
    const backupPath = getBackupPath(backupId);
 ...
 */
export async function restoreFromMigrationBackup(backupId, components = ['all']) {
    const result = {
        success: false,
        restoredFiles: [],
        errors: [],
    ...
    for (const file of manifest.files) {
        const shouldRestore = shouldRestoreAll ||
 ...
 */
export async function deleteMigrationBackup(backupId) {
    const backupPath = getBackupPath(backupId);
    try {
 ...
 */
export async function cleanupOldBackups(keepCount = 5) {
    const result = {
        deleted: [],
        errors: [],
    };
 ...
 */
export async function getBackupStats(backupId) {
    const manifest = await getMigrationBackup(backupId);
    if (!manifest)
        return null;
    const backupPath = getBackupPath(backupId);
 ...
 */
export async function createSelectiveBackup(reason, components) {
    const backupId = generateBackupId();
    const backupPath = getBackupPath(backupId);
    const claudeDir = getClaudeDir();
    // Detect current state
    ...
    for (const file of allFiles) {
        const shouldInclude = components.includes('all') ||
            (components.includes('agents') && file.startsWith('agents/')) ||
            (components.includes('hooks') && file.startsWith('hooks/')) ||
            (components.includes('skills') && file.startsWith('skills/')) ||
            (components.includes('settings') && file === 'settings.json') ||

...
import { getClaudeDir, getFileHash, detectMCPServers, } from './project-detector.js';
const execAsync = promisify(exec);
/**
 * Run a validation check
 */
async function runCheck(name, category, checkFn) {
    try {
 ...
 */
async function validateClaudeDirectory() {
    const checks = [];
 ...
 */
async function validateFileIntegrity(expectedHashes) {
    const checks = [];
    const claudeDir = getClaudeDir();
 ...
 */
async function validatePreservedSettings(originalSettings) {
    const checks = [];
    const claudeDir = getClaudeDir();
    const settingsPath = path.join(claudeDir, 'settings.json');
    try {
        ...
        if (originalSettings.apiTokens) {
            for (const [token, wasPresent] of Object.entries(originalSettings.apiTokens)) {
                if (wasPresent) {
                    const isPresent = !!(currentSettings[token] ||
        ...
        if (originalSettings.allowedTools && originalSettings.allowedTools.length > 0) {
            ...
            const allPreserved = originalSettings.allowedTools.every((tool) => currentTools.includes(tool));
            checks.push({
 ...
 */
export async function validateMigration(_migrationResult, expectedHashes, originalSettings) {
    const allChecks = [];
 ...
 */
export function suggestRemediation(result) {
    const suggestions = [];
    for (const check of result.errors) {

...
export function validateConfig(config) {
    ...
    if (!c.components || Object.keys(c.components).length === 0) {
 ...
 */
export function isConfigOutdated(config) {
 ...
 */
export async function getComponent(name, type) {
 ...
 * Remove a component
 */
export async function removeComponent(name, type) {
    const result = await loadConfig();
    if (!result.success || !result.config) {
        return {
            success: false,

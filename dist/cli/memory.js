...
import { storeMemory, queryMemories, deleteMemories, getMemoryStats, cleanupExpiredMemories, } from '../heimdall/storage-manager.js';
import { optimizedQuery, } from '../heimdall/query-optimizer.js';
import { runFullHealthCheck, getQuickStatus, getDiagnostics, } from '../heimdall/health-checker.js';
import { validateTags, TAG_PREFIXES, } from '../heimdall/tagging.js';
import { isHeimdallReady, installHeimdall, } from '../heimdall/installer.js';
...
export function createMemoryCommand() {
    ...
    memory
        ...
        if (options.tags) {
            tags.push(...options.tags.split(',').map((t) => t.trim()));
        }
        const memoryTypes = options.type ? [options.type] : undefined;
    ...
    memory
        ...
        const result = await optimizedQuery(query, {
            ...
            priorityTypes: options.type ? [options.type] : undefined,
    ...
    memory
        ...
        if (!ready)
            return;
        const rawTags = options.tags.split(',').map((t) => t.trim());
 ...
 */
async function ensureHeimdallReady() {

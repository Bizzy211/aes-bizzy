 ...
 */
function getCredentialRequirements(mode) {
    switch (mode) {
        case 'structural':
 ...
 */
function validateCredentials(env, requirements) {
    const missing = [];
    const found = [];
 ...
 */
function getCredentialHint(key) {
 ...
 */
async function runTests(mode, envFile, cwd) {
    // Build docker-compose command
    const args = ['-f', 'docker-compose.test.yml'];
 ...
 * Run cleanup command
 */
async function runCleanup(cwd) {
    const spinner = ora('Cleaning up test containers...').start();
    const result = await executeCommand('docker-compose', ['-f', 'docker-compose.test.yml', 'down', '-v'], { cwd, silent: true });
    if (result.exitCode === 0) {

 ...
 */
export function matchAgents(options: AgentMatchOptions): AgentMatch[] {
  const {
    text,
    maxAgents = 5,
    minScore = 20,
...
export function getAgentsByCategory(category: AgentCategory): AgentType[] {
  return AVAILABLE_AGENTS.filter(
    agent => AGENT_DEFINITIONS[agent].category === category
  );
}
 ...
 */
export function getRelatedAgents(agent: AgentType): AgentType[] {
 ...
 */
export function getRecommendedAgents(projectDescription: string): AgentType[] {
 ...
 */
export function getAgentDefinition(agent: AgentType): AgentDefinition | undefined {

 ...
 */
function analyzeProjectForAgents(description?: string): string[] {
  if (!description) {
 ...
 */
function generateSuggestedTasks(context: Partial<KickoffContext>): string[] {
  const tasks: string[] = [];
 ...
 */
export async function createKickoffContext(
  options: CreateKickoffContextOptions
): Promise<CreateKickoffContextResult> {
  const {
    projectName,
    projectPath,
    projectDescription,
 ...
 */
function generateKickoffMarkdown(context: KickoffContext): string {
  const lines: string[] = [
 ...
 */
export async function readKickoffContext(projectPath: string): Promise<KickoffContext | null> {
  ...
  try {
    ...
    return JSON.parse(content) as KickoffContext;
  } catch (error) {
 ...
 */
export function isKickoffModeEnv(): boolean {
 ...
 */
export function hasKickoffContextFile(projectPath: string): boolean {
  const contextPath = path.join(projectPath, '.claude', KICKOFF_CONTEXT_FILE);
 ...
 */
export async function detectKickoffMode(projectPath: string): Promise<KickoffModeDetection> {
 ...
 */
export function generateKickoffWorkflow(context: KickoffContext): KickoffWorkflowStep[] {
  const steps: KickoffWorkflowStep[] = [];
  let stepNum = 1;
 ...
 */
export function generateKickoffPrompt(context: KickoffContext): string {
 ...
 */
export function setKickoffModeEnv(enabled: boolean): void {
  if (enabled) {
 ...
 */
export async function clearKickoffContext(projectPath: string): Promise<boolean> {
  const claudeDir = path.join(projectPath, '.claude');

 ...
 */
export function hasAllClaudeEntries(content: string): boolean {
 ...
 */
export function getMissingEntries(content: string, entries: string[]): string[] {
...
export async function updateGitignore(
  projectPath: string,
  entries: string[] = CLAUDE_GITIGNORE_ENTRIES
): Promise<{
  success: boolean;
  created: boolean;
  entriesAdded: number;
 ...
 */
export async function removeClaudeEntries(
  projectPath: string
  ...
  try {
    ...
    const entriesToRemove = new Set(
      CLAUDE_GITIGNORE_ENTRIES.map((e) => e.trim()).filter((e) => e)
    );

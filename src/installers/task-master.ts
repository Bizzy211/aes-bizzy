...
import type {
  ...
  ToolTier,
  ModelProvider,
  TaskMasterInstallResult,
  MCPServerStatus,
  ModelConfig,
} from '../types/task-master.js';
import {
  AVAILABLE_MODELS,
  TOOL_TIERS,
  PROVIDER_ENV_VARS,
 ...
 */
export function getModelConfig(modelId: TaskMasterModel): ModelConfig | undefined {
 ...
 */
export function getModelProvider(modelId: TaskMasterModel): ModelProvider | undefined {
 ...
 */
export async function isTaskMasterInstalled(): Promise<MCPServerStatus> {
  try {
 ...
 */
export async function selectModel(skipPrompt = false): Promise<TaskMasterModel | undefined> {
  if (skipPrompt) {
  ...
  return selected as TaskMasterModel;
}

/**
 * Select tool tier interactively
 */
export async function selectToolTier(skipPrompt = false): Promise<ToolTier> {
  if (skipPrompt) {
    return 'core';
  }

  const tierOptions = Object.entries(TOOL_TIERS).map(([tier, config]) => ({
    value: tier as ToolTier,
 ...
 * Upgrade tool tier
 */
export async function upgradeToolTier(tier: ToolTier): Promise<boolean> {
  try {
 ...
 */
export async function validateTaskMaster(): Promise<{
  valid: boolean;
  error?: string;
}> {
  try {
 ...
 */
export function getToolTierInfo(tier: ToolTier): typeof TOOL_TIERS[ToolTier] {

...
import type { ComponentType } from '../types/repo-sync.js';
import type {
  UpdateOptions,
  UpdateResult,
  ComponentUpdate,
 ...
 */
function categorizeChangedFiles(files: string[]): ChangedFile[] {
  const componentDirs: Record<string, ComponentType> = {
    'agents/': 'agents',
    'hooks/': 'hooks',
    'skills/': 'skills',
    'scripts/': 'scripts',
 ...
 */
function createChangelogSummary(changedFiles: ChangedFile[]): string[] {
 ...
 */
function printUpdateSummary(result: UpdateResult, options: UpdateOptions): void {
  console.log();
...
export async function runUpdate(options: UpdateOptions = {}): Promise<UpdateResult> {
  ...
  try {
    ...
    const componentsToSync: ComponentType[] = options.component
      ? [options.component]
      : options.all
        ? ['agents', 'hooks', 'skills', 'scripts', 'slash-commands']
        : [...new Set(relevantChanges.map((f) => f.componentType).filter(Boolean))] as ComponentType[];
    ...
    for (const synced of syncResult.synced) {
      ...
      const action: ComponentUpdate['action'] =
        synced.action === 'merged' ? 'updated' : synced.action;

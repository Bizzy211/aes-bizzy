...
import {
  ...
  getMappingStats,
  getAutomationLog,
} from '../integrations/github-automation/index.js';
import type {
 ...
 */
function getToken(options: GitHubCommandOptions): string | null {
 ...
 */
function formatAgentMatch(match: AgentMatch): string {
  const colorMap = {
    high: chalk.green,
    medium: chalk.yellow,
    low: chalk.red,
 ...
 */
function formatTriageResult(result: TriageResult): string {
  const lines: string[] = [
...
export async function assignCommand(
  ...
  const config: Partial<GitHubAutomationConfig> = {
 ...
 */
export async function batchTriageCommand(
  repoStr: string,
  options: GitHubCommandOptions
): Promise<void> {
  const token = getToken(options);
  ...
  if (withMatches.length > 0) {
    ...
    for (const result of withMatches.slice(0, 5)) {
      ...
      if (best) {
        console.log(
          `  #${result.issue.number}: ${result.issue.title.slice(0, 40)}... → ${chalk.cyan(best.agentName)}`
        );
      }
    }
  }
 ...
 */
export async function batchAssignCommand(
  repoStr: string,
  options: GitHubCommandOptions
): Promise<void> {
  const token = getToken(options);
  ...
  const results = await batchAssignIssues(
    parsed.owner,
    parsed.repo,
    issues,
    token,
...
export async function listAgentsCommand(
  ...
  if (options.json) {
    const data: Record<string, { description: string; specializations: string[]; keywords: string[] }> = {};
 ...
 */
export async function mappingStatsCommand(
  options: GitHubCommandOptions
): Promise<void> {

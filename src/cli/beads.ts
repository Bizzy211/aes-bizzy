...
import {
  ...
  syncBeads,
  getAssignedAgent,
} from '../beads/assignment.js';
import type { BeadStatus } from '../types/beads.js';
 ...
 */
export function createBeadsCommand(): Command {
  ...
  beads
    ...
    .action(async (options) => {
      const result = await getReadyBeads({
        ...
        status: options.status as BeadStatus,
        json: beads.opts().json,
      });
    ...
    .action(async (options) => {
      ...
      if (beads.opts().json) {
        ...
        for (const bead of result.beads) {
          ...
          console.log(
            `  ${statusIcon} [${chalk.cyan(bead.id)}] ${bead.title}`
          );
          if (assignedAgent) {
            console.log(chalk.dim(`       → ${assignedAgent}`));
          }

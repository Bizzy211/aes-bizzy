 ...
 */
async function initTaskMaster(projectPath: string): Promise<boolean> {
  try {
    const result = await executeCommand(
      'npx',
 ...
 * Initialize Beads
 */
async function initBeads(projectPath: string): Promise<boolean> {
  try {
...
export async function initProject(
  ...
  const template: ProjectTemplate = options.template || 'basic';
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      if (!options.skipGit) logger.info('  Git: initialized');
      if (options.github) logger.info('  GitHub: repository created');
      if (options.taskmaster) logger.info('  Task Master: initialized');
      if (options.beads) logger.info('  Beads: initialized');
    ...
    if (options.github) {
      ...
      if (repoUrl) {
        ...
        result.warnings.push(
          'Failed to create GitHub repository. Make sure gh CLI is installed and authenticated.'
        );
      }
    }
    ...
    if (options.taskmaster) {
      ...
      if (tmInit) {
        ...
        result.warnings.push(
          'Failed to initialize Task Master. You can run "task-master init" manually.'
        );
      }
    }
    ...
    if (options.beads) {
      ...
      if (beadsInit) {
        ...
        result.warnings.push(
          'Failed to initialize Beads. You can run "bd init" manually.'
        );
      }
    }
 ...
 */
export function printProjectResult(result: ProjectResult): void {
  console.log();

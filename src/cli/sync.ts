...
/**
 * Manifest structure for installation configuration
 */
interface Manifest {
  name: string;
  version: string;
 ...
 */
function loadManifest(manifestName: string): Manifest | null {
 ...
 */
function getManifestFiles(manifest: Manifest): string[] {
  const files: string[] = [];
  ...
  for (const [, component] of Object.entries(manifest.components)) {
    ...
    for (const [, value] of Object.entries(component)) {
      if (typeof value === 'object' && value !== null && 'files' in value && 'path' in value) {
        const nested = value as { path: string; files: string[] };
 ...
 */
function listBackups(): { timestamp: string; path: string }[] {
  ...
  const backups: { timestamp: string; path: string }[] = [];
 ...
 */
async function restoreFromBackup(timestamp: string): Promise<SyncResult> {
 ...
 */
function displayManifestSummary(manifest: Manifest): void {
  console.log('');
  ...
  for (const [category, component] of Object.entries(manifest.components)) {
    if (component.count) {
      ...
      counts[category] = component.totalCount as number;
    }
  }

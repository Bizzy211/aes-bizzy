 ...
 */
function getConfig(): HeimdallConfig {
...
export async function queryMemories(
  ...
  try {
    ...
    const memories: HeimdallMemory[] = data.result.map(result => ({
      id: String(result.id),
      content: String(result.payload.content),
      tags: result.payload.tags as string[],
      timestamp: String(result.payload.timestamp),
      relevanceScore: result.score,
      agentName: result.payload.agentName as string | undefined,
      taskId: result.payload.taskId as string | undefined,
      memoryType: result.payload.memoryType as HeimdallMemoryType | undefined,
      ttl: result.payload.ttl as number | undefined,
    }));
  ...
  try {
    if (request.memoryId) {
      ...
      const response = await fetch(
        `${config.qdrantUrl}/collections/${config.collectionName}/points/delete`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
 ...
 */
export async function storeErrorResolution(
  error: string,
  resolution: string,
  agentName: string,
  taskId?: string,
 ...
 */
export async function cleanupExpiredMemories(): Promise<DeleteMemoryResponse> {
  const config = getConfig();

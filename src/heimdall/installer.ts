...
import {
  HeimdallConfig,
  HeimdallInstallStatus,
  HeimdallInstallOptions,
  HeimdallInstallResult,
  QdrantConfig,
 ...
 */
export async function getInstallStatus(): Promise<HeimdallInstallStatus> {
  const issues: string[] = [];
 ...
 */
async function isHeimdallMcpConfigured(): Promise<boolean> {
 ...
 */
async function hasHeimdallConfig(): Promise<boolean> {
 ...
 */
export async function installHeimdall(
  options: HeimdallInstallOptions = {}
): Promise<HeimdallInstallResult> {
 ...
 * Uninstall Heimdall
 */
export async function uninstallHeimdall(
 ...
 */
function ensureHeimdallDirectory(): void {
 ...
 */
export function saveHeimdallConfig(config: HeimdallConfig): void {
  ensureHeimdallDirectory();
  const configPath = path.join(HEIMDALL_HOME_DIR, CONFIG_FILE);
 ...
 */
export function loadHeimdallConfig(): HeimdallConfig | null {
  const configPath = path.join(HEIMDALL_HOME_DIR, CONFIG_FILE);
  try {
    if (fs.existsSync(configPath)) {
      const content = fs.readFileSync(configPath, 'utf-8');
      return JSON.parse(content) as HeimdallConfig;
    }
  } catch {
 ...
 */
async function configureHeimdallMcp(_config: HeimdallConfig): Promise<boolean> {
 ...
 */
async function ensureDefaultCollection(config: HeimdallConfig): Promise<void> {
 ...
 */
export async function isHeimdallReady(): Promise<boolean> {
 ...
 */
export async function autoFixHeimdall(): Promise<{
  fixed: string[];
  remaining: string[];
}> {
  const status = await getInstallStatus();

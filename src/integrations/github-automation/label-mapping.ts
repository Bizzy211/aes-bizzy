 ...
 */
const DEFAULT_LABEL_MAPPINGS: LabelMapping[] = [
  // Bug-related labels
 ...
 */
let customMappings: LabelMapping[] = [];

/**
 * Get all label mappings (default + custom)
 */
export function getAllLabelMappings(): LabelMapping[] {
  ...
  const defaults = DEFAULT_LABEL_MAPPINGS.filter(
    (m) => !customLabels.has(m.label.toLowerCase())
  );
 ...
 */
export function getAgentsForLabel(label: string): string[] {
 ...
 */
export function getAgentsForLabels(labels: string[]): Map<string, number> {
 ...
 */
export function addCustomMapping(mapping: LabelMapping): void {
  // Remove existing custom mapping for this label if exists
  customMappings = customMappings.filter(
    (m) => m.label.toLowerCase() !== mapping.label.toLowerCase()
  );
 ...
 */
export function removeCustomMapping(label: string): boolean {
 ...
 */
export function setCustomMappings(mappings: LabelMapping[]): void {
 ...
 */
export function getCustomMappings(): LabelMapping[] {
 ...
 */
export function getDefaultMappings(): LabelMapping[] {
 ...
 */
export async function suggestMappingsForLabel(
  label: string,
  agentsDir?: string
): Promise<string[]> {
  const capabilities = await loadAgentCapabilities(agentsDir);
 ...
 */
export async function getBestAgentForLabels(
  labels: string[],
  agentsDir?: string
): Promise<string | null> {
...
export function importMappings(json: string): { success: boolean; error?: string } {
  try {
    const mappings = JSON.parse(json) as LabelMapping[];

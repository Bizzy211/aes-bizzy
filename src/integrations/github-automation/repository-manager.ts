 ...
 */
export const DEFAULT_AGENT_LABELS: AgentLabel[] = [
 ...
 */
export const WORKFLOW_LABELS: AgentLabel[] = [
...
export const DEFAULT_MILESTONE_PHASES: MilestonePhase[] = [
  {
    ...
    description:
      'Initial project setup, architecture decisions, and core infrastructure',
    priority: 'high',
  },
  {
    title: 'Core Development',
    description:
      'Main feature implementation, API development, and core functionality',
    priority: 'high',
  },
  {
    title: 'Testing & QA',
  ...
  },
  {
    ...
    description:
      'Deployment preparation, documentation, and final polish',
    priority: 'medium',
  },
];
 ...
 */
function calculateDueDate(phaseIndex: number): Date {
  const now = new Date();
...
export async function initializeRepo(
  ...
  try {
    ...
    if (options.readme) {
      ...
      const response = await withRetry(() =>
        githubRequest<{ commit: { sha: string } }>(
    ...
    if (options.gitignore) {
      ...
      if (typeof options.gitignore === 'string') {
        ...
        try {
          const template = await githubRequest<{ source: string }>(
            `/gitignore/templates/${options.gitignore}`,
            token
          );
...
`;
    ...
    if (options.license) {
      try {
        const licenseResponse = await githubRequest<{ body: string }>(
          `/licenses/${options.license.toLowerCase()}`,
          token
        );
 ...
 */
export async function createDefaultLabels(
  owner: string,
  repo: string,
  token: string
...
export async function createMilestones(
  ...
  const milestonesCreated: Array<{ title: string; number: number; url: string }> = [];
  ...
  try {
    for (let i = 0; i < phases.length; i++) {
      ...
      const response = await withRetry(() =>
        githubRequest<{ number: number; html_url: string }>(
...
export async function setupRepository(
  ...
  try {
    ...
    const initResult = await initializeRepo(
      owner,
      repo,
      {
        readme: true,
    ...
    if (config.milestones && config.milestones.length > 0) {
      const milestoneResult = await createMilestones(
        owner,
        repo,
        config.milestones,
        token
    ...
    if (config.initialFiles && config.initialFiles.length > 0) {
      const uploadResult = await pushProjectFiles(
        owner,
        repo,
        config.initialFiles,
        token

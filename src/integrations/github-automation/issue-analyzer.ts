 ...
 */
export function extractLabelKeywords(labels: Array<{ name: string }>): string[] {
...
async function scoreAgent(
  ...
  for (const labelKeyword of labelKeywords) {
    if (capability.specializations.some((s) =>
      s.toLowerCase() === labelKeyword.toLowerCase()
    )) {
...
export async function analyzeIssue(
  ...
  for (const [agentName, capability] of capabilities) {
    const { score, matchedKeywords } = await scoreAgent(
      capability,
      allKeywords,
      labelKeywords
    );
 ...
 */
function suggestLabels(keywords: string[], existingLabels: string[]): string[] {
 ...
 */
export function extractIssueKeywords(issue: GitHubIssue): string[] {
  const titleKeywords = extractKeywords(issue.title);
  const bodyKeywords = extractKeywords(issue.body || '');
  const labelKeywords = extractLabelKeywords(issue.labels);

 ...
 */
export function displayInstallationHeader(detection: DetectionResult): void {
  console.log();
 ...
 */
export function getStepStatuses(detection: DetectionResult): Array<{
  stepNumber: number;
  stepName: string;
  status: 'complete' | 'update' | 'missing' | 'pending';
  details?: string;
  ...
  steps.push({
    ...
    status: detection.settingsJson.exists ? 'complete' as const : 'missing' as const,
  ...
  steps.push({
    ...
    status: detection.agents.exists
      ? (detection.agents.count > 0 ? 'update' as const : 'pending' as const)
      : 'missing' as const,
  ...
  steps.push({
    ...
    status: detection.beads.installed ? 'complete' as const : 'missing' as const,
  ...
  steps.push({
    ...
    status: detection.taskMaster.configured ? 'complete' as const : 'missing' as const,
 ...
 */
export async function runAdaptiveInitFlow(
  detection?: DetectionResult
): Promise<AdaptiveInitResult | null> {
  ...
  // Initialize result
  const result: AdaptiveInitResult = {
  ...
  if (mode !== 'fresh-install') {
    ...
    result.mergeStrategy = strategy as MergeStrategy;
  }
...
export async function runMigrationWizard(
  ...
  if (!flowResult.skipBackup) {
    ...
    try {
      const backup = await createPreMigrationBackup(
        `Migration: ${flowResult.mode}`,
        flowResult.mode === 'fresh-install' ? 'full' : 'partial',
        flowResult.conflicts
      );
  ...
  if (flowResult.components.agents) result.componentsUpdated.push('agents');
  if (flowResult.components.hooks) result.componentsUpdated.push('hooks');
  if (flowResult.components.skills) result.componentsUpdated.push('skills');
  if (flowResult.components.mcpServers) result.componentsUpdated.push('mcpServers');
  if (flowResult.components.settings) result.componentsUpdated.push('settings');
 ...
 */
async function resolveConflictInteractive(conflict: Conflict): Promise<ConflictResolution | null> {
  console.log();
  ...
  return action as ConflictResolution;
}
...
export async function generateMigrationPlan(
  ...
  if (components.agents) plan.componentsToUpdate.push('agents');
  if (components.hooks) plan.componentsToUpdate.push('hooks');
  if (components.skills) plan.componentsToUpdate.push('skills');
  if (components.mcpServers) plan.componentsToUpdate.push('mcpServers');
  if (components.settings) plan.componentsToUpdate.push('settings');
  ...
  const newMCPServers: string[] = [];

  plan.conflicts = await detectConflicts(
    detection,
    newAgents,
    newHooks,
    newSkills,
 ...
 */
export function exportMigrationPlan(plan: MigrationPlan): string {
  const exportable = {
    ...plan,

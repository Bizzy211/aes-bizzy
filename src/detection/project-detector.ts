...
import type {
  ...
  LegacySetupResult,
  AgentInfo,
  AgentsDetectionResult,
  HookInfo,
  HooksDetectionResult,
  SkillInfo,
  SkillsDetectionResult,
  MCPServerInfo,
  MCPServersDetectionResult,
 ...
 */
const KNOWN_AGENT_HASHES: Map<string, string> = new Map();

/**
 * Known hook hashes from the repository
 */
const KNOWN_HOOK_HASHES: Map<string, string> = new Map();

/**
 * Known skill hashes from the repository
 */
const KNOWN_SKILL_HASHES: Map<string, string> = new Map();
 ...
 */
async function parseMCPServers(settingsPath: string): Promise<string[]> {
  try {
 ...
 */
async function detectBeads(): Promise<{ installed: boolean; version?: string }> {
  try {
 ...
 */
async function detectTaskMaster(projectRoot: string): Promise<{ configured: boolean; model?: string }> {
  try {
 ...
 */
async function parseEcosystem(ecosystemPath: string): Promise<{ exists: boolean; version?: string }> {
  try {
 ...
 */
export async function detectExistingSetup(projectRoot?: string): Promise<DetectionResult> {
 ...
 */
export async function analyzeExistingConfig(settingsPath: string): Promise<ConfigAnalysis> {
  ...
  try {
    ...
    result.missingMCPServers = REQUIRED_MCP_SERVERS.filter(
      (server) => !result.existingMCPServers.includes(server)
    );
 ...
 */
async function getAgentFiles(agentsDir: string): Promise<AgentInfo[]> {
 ...
 */
async function getHookFiles(hooksDir: string): Promise<HookInfo[]> {
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isFile()) {
        const ext = path.extname(entry.name).toLowerCase();
        let hookType: HookInfo['type'] | null = null;

        if (ext === '.py') hookType = 'python';
        else if (ext === '.sh') hookType = 'bash';
        else if (ext === '.ps1') hookType = 'powershell';
        else if (ext === '.bat') hookType = 'batch';
 ...
 */
async function getSkillFiles(skillsDir: string): Promise<SkillInfo[]> {
 ...
 */
export async function detectAgents(agentsDir?: string): Promise<AgentsDetectionResult> {
  const dir = agentsDir || path.join(getClaudeDir(), 'agents');
  const agentFiles = await getAgentFiles(dir);

  const result: AgentsDetectionResult = {
    installed: [],
    modified: [],
    unknown: [],
  };
 ...
 */
export async function detectHooks(hooksDir?: string): Promise<HooksDetectionResult> {
  const dir = hooksDir || path.join(getClaudeDir(), 'hooks');
  const hookFiles = await getHookFiles(dir);

  const result: HooksDetectionResult = {
    active: [],
    inactive: [],
    custom: [],
  };
 ...
 */
export async function detectSkills(skillsDir?: string): Promise<SkillsDetectionResult> {
  const dir = skillsDir || path.join(getClaudeDir(), 'skills');
  const skillFiles = await getSkillFiles(dir);

  const result: SkillsDetectionResult = {
    installed: [],
    modified: [],
    unknown: [],
  };
 ...
 */
export async function detectMCPServers(): Promise<MCPServersDetectionResult> {
  const result: MCPServersDetectionResult = {
    configured: [],
    orphaned: [],
    missing: [],
  };
  ...
  try {
    ...
    result.missing = REQUIRED_MCP_SERVERS.filter(
 ...
 */
export async function detectLegacySetup(): Promise<LegacySetupResult> {
  const claudeDir = getClaudeDir();

  const result: LegacySetupResult = {
    isLegacy: false,
    migrationPath: 'none',
    oldStructure: {
 ...
 */
export function updateKnownAgentHashes(hashes: Map<string, string>): void {
 ...
 */
export function updateKnownHookHashes(hashes: Map<string, string>): void {
 ...
 */
export function updateKnownSkillHashes(hashes: Map<string, string>): void {

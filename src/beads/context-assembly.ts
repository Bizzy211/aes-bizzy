...
const CONTEXT_TYPES_FOR_AGENTS: ContextType[] = ['decision', 'architecture', 'pattern', 'learning'];
 ...
 */
export async function assembleContextForAgent(
  taskId: string,
  agentType: string,
  options: {
    scope?: ContextScope;
  ...
  const relevantBeads = allBeads.filter(b =>
    CONTEXT_TYPES_FOR_AGENTS.includes(b.contextType)
  );
 ...
 */
async function getTaskMasterContext(taskId: string): Promise<TaskContext | undefined> {
  try {
    ...
    const context: TaskContext = {
      ...
      dependencies: taskData.dependencies?.map((d: number | string) => String(d)),
    };
 ...
 */
function deduplicateBeads(beads: ContextBead[]): ContextBead[] {
...
export function formatContextPrompt(
  ...
  if (beadsContext.length > 0) {
    ...
    const typeOrder: ContextType[] = ['decision', 'architecture', 'pattern', 'learning', 'blocker', 'handoff'];
    for (const type of typeOrder) {
      const typeBeads = byType.get(type);
      if (!typeBeads || typeBeads.length === 0) continue;

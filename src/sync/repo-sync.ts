...
import type { EcosystemConfig, EcosystemComponentType } from '../types/ecosystem.js';
import {
  COMPONENT_TYPES,
  COMPONENT_DIRS,
  DEFAULT_REPO_URL,
 ...
 */
function getRelativeComponentPath(component: ComponentFile): string {
 ...
 */
function getTempCloneDir(): string {
 ...
 */
async function loadEcosystemConfig(): Promise<EcosystemConfig> {
 ...
 */
async function saveEcosystemConfig(config: EcosystemConfig): Promise<void> {
  const configPath = getEcosystemConfigPath();
 ...
 */
async function cloneOrUpdateRepo(token: string): Promise<{ git: SimpleGit; repoPath: string }> {
 ...
 */
async function getRepoMetadata(git: SimpleGit): Promise<RepoMetadata> {
 ...
 */
async function discoverComponents(repoPath: string): Promise<AvailableComponents> {
  const result: AvailableComponents = {
    agents: [],
    hooks: [],
    skills: [],
    scripts: [],
...
async function copyComponent(
  ...
  let action: SyncedComponent['action'] = 'created';
...
): Promise<ComponentFile[] | undefined> {
    ...
    if (interactive) {
      try {
        const confirm = await confirmAction(
          `Conflict: ${conflict.file.name} already exists. Overwrite?`
        );
 ...
 */
export async function syncPrivateRepo(
  ...
  try {
    ...
    if (!options.dryRun && result.synced.length > 0) {
      ...
      const syncedByType: Record<EcosystemComponentType, string[]> = {
        agents: [],
        hooks: [],
        skills: [],
        scripts: [],
 ...
 */
export async function pullLatestChanges(token: string): Promise<PullResult> {
 ...
 */
export async function getAvailableComponents(token: string): Promise<AvailableComponents | null> {
  try {

#!/usr/bin/env python3
"""
Security Engineer Agent - Multi-Agent Development Team
Specializes in Security patterns, vulnerability assessment, compliance, secure coding.
"""
import sys
import os
...
class SecurityengineerAgent(BaseAgent):
    """
    Security Engineer Agent - The Security patterns specialist.
    
    Responsibilities:
    - Security patterns, vulnerability assessment, compliance, secure coding
    ...
    def __init__(self):
        super().__init__(
            ...
            specialization="Security patterns, vulnerability assessment, compliance, secure coding"
        )
        
        # Security Engineer specific state
        self.state.setdefault("reviews", [])
        self.state.setdefault("recommendations", [])
        self.state.setdefault("metrics", {})
        
        # Initialize message processor
        self.message_processor = AgentMessageProcessor("security-engineer")
    
    def get_agent_personality(self) -> Dict[str, str]:
        """Security Engineer personality traits"""
        return {
            "voice_type": "security_focused_authoritative",
            "communication_style": "technical_expert",
            "speaking_pace": "measured",
            "personality_traits": ['security-minded', 'thorough', 'compliance-focused', 'risk-aware', 'protective'],
            "catchphrases": ['Is this secure?', 'What about authentication?', 'Check for vulnerabilities', 'Security first']
        }
    
    def process_hook_input(self, tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        """Process hook input and provide security engineer expertise"""
    ...
    def handle_file_operations(self, tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if issues:
            ...
            message_parts = [f"🔐 Security Engineer: Reviewing {Path(file_path).name}"]
        ...
        return {
            "message": f"🔐 Security Engineer: {Path(file_path).name} looks good from security engineer perspective!",
            "block": False
        }
    
    def handle_bash_commands(self, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        """Handle bash commands relevant to security engineer"""
        ...
        if suggestions:
            message_parts = [f"🔐 Security Engineer: Command analysis"]
    ...
    def handle_general_guidance(self, tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        """Provide general security engineer guidance"""
        return {"message": "", "block": False}
    
    def is_relevant_file(self, file_path: str) -> bool:
        """Check if file is relevant to security engineer specialization"""
        file_path_lower = file_path.lower()
        
        # Check extensions
        relevant_extensions = ['.py', '.js', '.java', '.go', '.cs']
        if any(file_path_lower.endswith(ext) for ext in relevant_extensions):
            return True
        
        # Check directories
        relevant_dirs = ['auth/', 'security/', 'api/']
    ...
    def analyze_content(self, file_path: str, content: str, tool_name: str) -> List[Dict[str, Any]]:
        """Analyze content for security engineer specific issues"""
    ...
    def get_command_suggestions(self, command: str) -> List[str]:
        """Get command suggestions specific to security engineer"""
    ...
    def handle_file_change_from_team(self, data: Dict[str, Any]):
        ...
        if self.is_relevant_file(file_path):
            ...
            self.send_message(
                "project-manager",
                "security_engineer_review_complete",
                {
                    "file_path": file_path,
                    "status": "reviewed",
                    "agent": "security-engineer"
    ...
    def generate_recommendations(self, file_path: str, review_type: str) -> List[str]:
        """Generate recommendations specific to security engineer"""

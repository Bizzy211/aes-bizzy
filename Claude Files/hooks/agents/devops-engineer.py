#!/usr/bin/env python3
"""
Devops Engineer Agent - Multi-Agent Development Team
Specializes in CI/CD, containerization, deployment, infrastructure, monitoring.
"""
import sys
import os
...
class DevopsengineerAgent(BaseAgent):
    """
    Devops Engineer Agent - The CI/CD specialist.
    
    Responsibilities:
    - CI/CD, containerization, deployment, infrastructure, monitoring
    ...
    def __init__(self):
        super().__init__(
            ...
            specialization="CI/CD, containerization, deployment, infrastructure, monitoring"
        )
        
        # Devops Engineer specific state
        self.state.setdefault("reviews", [])
        self.state.setdefault("recommendations", [])
        self.state.setdefault("metrics", {})
        
        # Initialize message processor
        self.message_processor = AgentMessageProcessor("devops-engineer")
    
    def get_agent_personality(self) -> Dict[str, str]:
        """Devops Engineer personality traits"""
        return {
            "voice_type": "infrastructure_focused_male",
            "communication_style": "technical_expert",
            "speaking_pace": "measured",
            "personality_traits": ['systematic', 'reliability-focused', 'automation-minded', 'scalability-oriented', 'monitoring-focused'],
            "catchphrases": ["Let's automate this", 'Is this scalable?', 'What about monitoring?', 'Deploy with confidence']
        }
    
    def process_hook_input(self, tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        """Process hook input and provide devops engineer expertise"""
    ...
    def handle_file_operations(self, tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if issues:
            ...
            message_parts = [f"🏗️ Devops Engineer: Reviewing {Path(file_path).name}"]
        ...
        return {
            "message": f"🏗️ Devops Engineer: {Path(file_path).name} looks good from devops engineer perspective!",
            "block": False
        }
    
    def handle_bash_commands(self, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        """Handle bash commands relevant to devops engineer"""
        ...
        if suggestions:
            message_parts = [f"🏗️ Devops Engineer: Command analysis"]
    ...
    def handle_general_guidance(self, tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        """Provide general devops engineer guidance"""
        return {"message": "", "block": False}
    
    def is_relevant_file(self, file_path: str) -> bool:
        """Check if file is relevant to devops engineer specialization"""
        file_path_lower = file_path.lower()
        
        # Check extensions
        relevant_extensions = ['.yml', '.yaml', '.json', '.sh']
        if any(file_path_lower.endswith(ext) for ext in relevant_extensions):
            return True
        
        # Check directories
        relevant_dirs = ['deploy/', 'ci/', 'docker/', '.github/']
    ...
    def analyze_content(self, file_path: str, content: str, tool_name: str) -> List[Dict[str, Any]]:
        """Analyze content for devops engineer specific issues"""
    ...
    def get_command_suggestions(self, command: str) -> List[str]:
        """Get command suggestions specific to devops engineer"""
    ...
    def handle_file_change_from_team(self, data: Dict[str, Any]):
        ...
        if self.is_relevant_file(file_path):
            ...
            self.send_message(
                "project-manager",
                "devops_engineer_review_complete",
                {
                    "file_path": file_path,
                    "status": "reviewed",
                    "agent": "devops-engineer"
    ...
    def generate_recommendations(self, file_path: str, review_type: str) -> List[str]:
        """Generate recommendations specific to devops engineer"""
